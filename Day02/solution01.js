...
function check(value) {
  ...
  for(let i = 0; i < entries.length; i++) {
    let total = original.filter(item => {
      return item === entries[i]
    })
    
    
    if(total.length === 2 && ! foundDbl) {
      twice.add(value)
      foundDbl = true
    } else if (total.length === 3 && ! foundTrip) {
      thrice.add(value)
      foundTrip = true
    }
  }  
...
let output = twice.size * thrice.size
